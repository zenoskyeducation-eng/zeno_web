...
app.get('/api/tle', async (req, res) => {
  ...
  try {
    ...
    const celestrakRes = await fetch(celestrakUrl);
    ...
    return res.json({ tles });
  } catch (err) {
    console.error('Error proxying CelesTrak TLE:', err);
    return res.json({ tles: [] });
  }
});
  ...
  try {
    const adminFetch = fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: SENDER_EMAIL,
        to: [ADMIN_RECEIVER],
        subject: `New Transmission - New Inquiry from ${name} (${service})`,
        html: adminEmailHtml
      })
    });
...
app.listen(PORT, () => {
  console.log(`Zeno-Sky Transmission & TLE Proxy Server running on port ${PORT}`);
});
